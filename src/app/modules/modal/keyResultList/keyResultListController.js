...
appModule.controller('keyResultListController', KeyResultListController);

KeyResultListController.$inject = ['$uibModalInstance', 'userService', 'keyResultService', '$window', 'items', 'objectiveService', 'uploadService', 'commentService']

function KeyResultListController($uibModalInstance, userService, keyResultService, $window, items, objectiveService, uploadService, commentService) {
...
KeyResultListController.prototype.initialize = function () {
    var that = this
...
KeyResultListController.prototype.setNewValue = function (key, index) {
...
KeyResultListController.prototype.formatDate = function (date) {
    return moment(date).format('L')
}

KeyResultListController.prototype.formatTime = function (date) {
    return moment(date).format('LTS')
}

KeyResultListController.prototype.getLastCheckInDate = function (key) {
    return this.formatDate(key._id.checkIns[key._id.checkIns.length - 1]._id.insertDate)
}

KeyResultListController.prototype.getLastCheckInTime = function (key) {
    return this.formatTime(key._id.checkIns[key._id.checkIns.length - 1]._id.insertDate)
}

KeyResultListController.prototype.save = function () {
    var that = this,
        keyResultPromises = [],
...
KeyResultListController.prototype.createCheckIn = function (keyResultPromises, file) {
    var that = this
...
KeyResultListController.prototype.fileSelected = function () {
...
KeyResultListController.prototype.getInitials = function (ownerObj) {
    var first,
        last
...
KeyResultListController.prototype.closeModal = function () {
    this.$uibModalInstance.close('cancel')
};

KeyResultListController.prototype.mixpanel = function (msg) {
    mixpanel.track(msg);
